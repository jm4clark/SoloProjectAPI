pipeline{
		...
		stage('--deploy--'){
                        steps{
                                sh "cd /"
				sh "pwd"
				sh "sudo cp /var/lib/jenkins/workspace/pipeline/KingGizzAPI/target/KingGizzAPI.war /home/jamesmclark96/scripts/wildfly-10.1.0.Final/standalone/deployments/"
                        }
                }
        }
}